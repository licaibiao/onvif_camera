...
typedef struct
{
	bool bRTSPState;	        /**RTSP状态**/
	int  s32SockFd;             /**RTSP socket**/
	int  s32StreamType;			/**主次码流类型**/
...
typedef struct
{
    bool bRTCPState;	            /**RTSP状态**/
    int  s32SockFd;                 /**RTSP socket**/
    int  s32StreamType;			    /**主次码流类型**/
    char arrs8SessionId[16];        /**RTCP会话ID**/
    char arrs8ServerIP[32];         /**RTCP服务端IP**/
    unsigned int u32SerRTCPPort;    /**服务端RTCP端口号**/
    unsigned int u32ClientRTCPPort; /**客户端RTCP端口号**/
}RTCP_STATUS_S;

typedef struct
{
    bool bRTPState;	                /**RTP状态**/
    int  s32RTPSockFd;              /**RTP socket**/
    char arrs8SessionId[16];        /**RTP会话ID**/
    char arrs8ServerIP[32];         /**RTCP服务端IP**/
    unsigned char s8StreamNum;      /**数据流号，用来申请对应的缓存队列**/
    unsigned int u32SerRTPPort;	    /**服务端RTP端口号**/
    unsigned int u32ClientRTPPort;  /**客户端RTP端口号**/
